...
//void write_myFile(const char *DMA_buffer);
...
//	if (fresult != FR_OK)
//	{
//		printf("Failed to get free space SD card \r\n");
//		//Print status
//		printf("SD card result = %d\r\n",fresult);
//		while(1);
//	}
...
//	UINT bytesWrote;
//
//	fresult = f_open(&file, "write2.txt", FA_WRITE | FA_READ | FA_CREATE_ALWAYS);//Revisar las declaraciones de FA_WRITE y todas las demas
//	if(fresult == FR_OK) {
...
//
//	 //Copy in a string
//	  strncpy((char*)buffer, "hola buenas noches 33", 22);
//	  fresult = f_write(&file, buffer, strlen(buffer), &bytesWrote);
//	  if(fresult == FR_OK) {
...
//void read_myFile(void){
//
//	fresult = f_open(&file, "write2.txt", FA_READ );//Revisar las declaraciones de FA_WRITE y todas las demas
//	if(fresult == FR_OK) {
...
////	UINT readbytes=0;
////	f_lseek(&file, 0);
////	fresult=f_read(&file, dataRD, 256, &readbytes);
//	//dataRD[readbytes]=0;
//	  TCHAR* rres = f_gets((TCHAR*)readBuf, 30, &file);
//	  if(rres != 0) {
//		printf("Read string from 'write2.txt' contents: %s\r\n", readBuf);
//	  } else {
